...
let tareaMasLarga = null;
let tiempoMasLargo = 0;
let programadorConTareaMasLarga = null;
...
programadores.forEach((programador) => {
  programador.tareas.forEach((tarea) => {
    ...
    if (tiempo > tiempoMasLargo) {
      ...
      tareaMasLarga = tarea;
      programadorConTareaMasLarga = programador;
    }
  });
});

console.log(
  `La tarea con mayor tiempo de desarrollo es "${tareaMasLarga.nombreTarea}" realizada por ${programadorConTareaMasLarga.nombre} y dura ${tareaMasLarga.tiempoDesarrollo}.`
);
